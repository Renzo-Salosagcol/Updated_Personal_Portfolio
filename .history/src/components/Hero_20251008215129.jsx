

export const Hero = () => {
  return (
    <main className="relative min-h-screen flex items-center justify-center max-w-10/12 rounded-md container z-0">
      <div className="z-0 relative gradient-border opacity-0 animate-fade-in">
        <div className="relative container flex justify-between gap-6 rounded-full p-6 bg-background">
        <div className="">
          <h1 className="relative max-w-max after:gradient-underline">Name</h1>
        </div>
        <div>
          <h1>Image</h1>
        </div>
      </div>
      </div>
    </main>
  )
}